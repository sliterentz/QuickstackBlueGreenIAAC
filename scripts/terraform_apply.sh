...
if ! sudo virsh pool-list --active | grep -q "$POOL_NAME"; then
...
if ! sudo virsh net-list --active | grep -q "$NETWORK_NAME"; then
    print_status "warn" "Network '$NETWORK_NAME' is not active. Starting..."
    if sudo virsh net-info "$NETWORK_NAME" 2>/dev/null | grep -q "Active:.*yes"; then
